...
void CVulkanSwapChain::Resize(uint32 NewWidth, uint32 NewHeight)
{
    ...
    verifyFunkinf(WaitIdleResult == vk::Result::eSuccess, "Failed to wait for the Vulkan logical device to idle before resizing the swapchain! ({})", vk::to_string(WaitIdleResult))
    ...
    for (const vk::Semaphore& OldRenderFinishedSemaphore : OldRenderFinishedSemaphores)
        m_LogicalDevice.destroySemaphore(OldRenderFinishedSemaphore);
    
    for (const vk::ImageView& OldSwapChainImageView : OldSwapChainImageViews)
        m_LogicalDevice.destroyImageView(OldSwapChainImageView);
...
FAcquiredFrame CVulkanSwapChain::AcquireNextImage()
{
    ...
    const vk::Result WaitForFencesResult = m_LogicalDevice.waitForFences(1, &InFlightFence, vk::True, std::numeric_limits<uint64>::max());
    ...
    const vk::Result ResetFencesResult = m_LogicalDevice.resetFences(1, &InFlightFence);
    verifyFunkinf(ResetFencesResult == vk::Result::eSuccess, "Failed to reset the in-flight fence! ({})", vk::to_string(ResetFencesResult))
...
void CVulkanSwapChain::CreateSwapChainAndDependents(const vk::Extent2D& RequestedSize, vk::SwapchainKHR OldSwapchain)
{
...
void CVulkanSwapChain::DestroySwapChainAndDependents()
{
    ...
    for (const vk::ImageView& SwapChainImageView : m_SwapChainImageViews)
        m_LogicalDevice.destroyImageView(SwapChainImageView);
...
void CVulkanSwapChain::CreateSwapChain(const vk::Extent2D& RequestedSize, vk::SwapchainKHR OldSwapchain)
{
    const auto SurfaceCapabilitiesResult = m_PhysicalDevice.getSurfaceCapabilitiesKHR(m_Surface);
    verifyFunkinf(SurfaceCapabilitiesResult.result == vk::Result::eSuccess, "Failed to query Vulkan surface capabilities! ({})", vk::to_string(SurfaceCapabilitiesResult.result))
    ...
    VK_CHECK_RESULT(m_LogicalDevice.createSwapchainKHR(SwapChainCreateInfo), m_SwapChain, "Failed to create Vulkan swapchain!")
    
    const auto SwapChainImagesResult = m_LogicalDevice.getSwapchainImagesKHR(m_SwapChain);
    verifyFunkinf(SwapChainImagesResult.result == vk::Result::eSuccess, "Failed to retrieve Vulkan swapchain images!")
    m_SwapChainImages = SwapChainImagesResult.value;
    
    m_SwapChainImageFormat = ChosenImageFormat;
    m_SwapChainExtent = ChosenExtent;
    
    LOG_DEBUG_TAG("VulkanRHI", "Created Vulkan swapchain with the following details:");
...
void CVulkanSwapChain::CreateImageViews()
{
    ...
    for (size_t i = 0; i < m_SwapChainImages.size(); i++)
    {
        ...
        VK_CHECK_RESULT(m_LogicalDevice.createImageView(ImageViewCreateInfo), m_SwapChainImageViews[i], "Failed to create Vulkan swapchain image view {}!", i)
    }
}
...
void CVulkanSwapChain::CreateRenderFinishedSemaphores()
{
    ...
    for (size_t i = 0; i < m_SwapChainImages.size(); i++)
        VK_CHECK_RESULT(m_LogicalDevice.createSemaphore(SemaphoreCreateInfo), m_RenderFinishedSemaphores[i], "Failed to create Vulkan render finished semaphore!")
}
...
void CVulkanSwapChain::CreateFrameSyncObjects()
{
    ...
    {
        VK_CHECK_RESULT(m_LogicalDevice.createSemaphore(SemaphoreCreateInfo), m_FrameSyncObjects[i].ImageAvailableSemaphore, "Failed to create Vulkan image-available semaphore {}!", i)
        VK_CHECK_RESULT(m_LogicalDevice.createFence(FenceCreateInfo), m_FrameSyncObjects[i].InFlightFence, "Failed to create Vulkan in-flight fence {}!", i)
    }
}
...
void CVulkanSwapChain::DestroyFrameSyncObjects()
{
    for (const auto& [ImageAvailableSemaphore, InFlightFence] : m_FrameSyncObjects)
    {
        ...
        m_LogicalDevice.destroyFence(InFlightFence);
    }
...
{
    const auto SurfaceFormatsResult = m_PhysicalDevice.getSurfaceFormatsKHR(m_Surface);
    verifyFunkinf(SurfaceFormatsResult.result == vk::Result::eSuccess && !SurfaceFormatsResult.value.empty(), "Failed to query Vulkan surface formats!")
...
vk::PresentModeKHR CVulkanSwapChain::SelectPresentMode() const
{
    const auto PresentModesResult = m_PhysicalDevice.getSurfacePresentModesKHR(m_Surface);
    verifyFunkinf(PresentModesResult.result == vk::Result::eSuccess && !PresentModesResult.value.empty(), "Failed to query Vulkan surface present modes!")
    ...
    if (bRequestVSync)
    {
        ...
        verifyFunkinf(bIsFIFOAvailable, "VK_PRESENT_MODE_FIFO_KHR is unexpectedly unavailable! This present mode is required to be supported per the Vulkan specification.")
