...
{
    CVulkanSwapChain& SwapChain = m_VulkanContext.GetMainWindowSwapChain();
    ...
    m_CurrentAcquiredFrame = AcquiredFrame;
...
{
    CVulkanSwapChain& SwapChain = m_VulkanContext.GetMainWindowSwapChain();
    if (!SwapChain.IsValid() || !m_CurrentAcquiredFrame.has_value())
        return;
    
    const FAcquiredFrame AcquiredFrame = m_CurrentAcquiredFrame.value();
    m_CurrentAcquiredFrame.reset();
